...
public class Binary_Tree_Level_Order_Traversal {
  ...
  private static List<List<Integer>> level_order_traversal_1(Node root) {
    ...
    LinkedList<Node> current_level_nodes_queue = new LinkedList<>();
    LinkedList<Node> next_level_nodes_queue = new LinkedList<>();

    if (root != null) {
      current_level_nodes_queue.addLast(root);
    }

    while (!current_level_nodes_queue.isEmpty()) {
      List<Integer> current_level_data = new ArrayList<>();

      while (!current_level_nodes_queue.isEmpty()) {
        Node first = current_level_nodes_queue.removeFirst();
        current_level_data.add(first.data);
        if (first.left != null) {
          next_level_nodes_queue.addLast(first.left);
        }
        if (first.right != null) {
          next_level_nodes_queue.addLast(first.right);
        }
      }

      data_all_levels.add(current_level_data);
      current_level_nodes_queue.addAll(next_level_nodes_queue);
      next_level_nodes_queue.clear();
    }
